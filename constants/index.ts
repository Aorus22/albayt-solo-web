// NAVIGATION
// test

export const NAV_LINKS = [
  { href: '/', key: 'beranda', label: 'Beranda' },
  { href: '/about-us', key: 'tentang_kami', label: 'Tentang Kami' },
  { href: '/paketpage', key: 'paket_albayt', label: 'Paket ALBAYT' },
  // { href: '/galeri', key: 'galeri', label: 'Galeri' },
  // { href: '/blogpage', key: 'blog', label: 'Blog' },
];

export const DATA_BANK = [
  {
      nama: 'BNI',
      rekening: "1484881240",
      img: '/logo-bni.jpg',
  },
  {
    nama: 'Redeem Code',
    rekening: "Upload redeem code pada bukti transfer",
    img: '/bank-default.svg',
  }
]

  export const HERO_SLIDE = [
    {
      key: 'slide1',
      img: '/Banner/01.jpg',
    },
    {
      key: 'slide2',
      img: '/Banner/02.jpg',
    },
    {
      key: 'slide3',
      img: '/Banner/03.jpg',
    },
    
  ]
  
  // DUMMY PACKAGE DATA
  // export const PACKAGE_DATA = [{
  //   "_id": {
  //     "$oid": "660f52bd9a7feaedf622ac38"
  //   },
  //   "key": "paket_1",
  //   "img": "/Poster/paket_1.jpeg",
  //   "harga": "Rp24.500.000,~",
  //   "tipe": "Quad",
  //   "title": "Napak Tilas Bumi Para Rasul",
  //   "jadwal": "11 Agustus 24",
  //   "durasi": "9 Hari",
  //   "hotel": 4,
  //   "totalseat": 40,
  //   "remainingseat": 10,
  //   "lokasiberangkat": "Surakarta"
  // },
  //   {
  //     "_id": {
  //       "$oid": "660f52bd9a7feaedf622ac39"
  //     },
  //     "key": "paket_2",
  //     "img": "/Poster/paket_2.jpeg",
  //     "harga": "Rp24.500.000,~",
  //     "tipe": "Quad",
  //     "title": "Program Umroh Awal Musim 2024 ",
  //     "jadwal": "27 Dec 24",
  //     "durasi": "9 Hari",
  //     "hotel": 5,
  //     "totalseat": 40,
  //     "remainingseat": 30,
  //     "lokasiberangkat": "Surakarta"
  //   },
  //   {
  //     "_id": {
  //       "$oid": "660f52bd9a7feaedf622ac3a"
  //     },
  //     "key": "paket_3",
  //     "img": "/Poster/paket_3.jpeg",
  //     "harga": "Rp24.500.000,~",
  //     "tipe": "Quad",
  //     "title": "Umroh Eksekutif Akhir Desember 2024 Plus City tour UIM + Museum Wahyu",
  //     "jadwal": "27 Dec 24",
  //     "durasi": "9 Hari",
  //     "hotel": 5,
  //     "totalseat": 40,
  //     "remainingseat": 10,
  //     "lokasiberangkat": "Surakarta"
  //   },
  //   {
  //     "_id": {
  //       "$oid": "660f52bd9a7feaedf622ac3b"
  //     },
  //     "key": "paket_4",
  //     "img": "/Poster/paket_4.jpeg",
  //     "harga": "Rp24.500.000,~",
  //     "tipe": "Quad",
  //     "title": "Umroh Eksekutif Akhir Desember 2024 Plus City tour UIM + Museum Wahyu",
  //     "jadwal": "27 Dec 24",
  //     "durasi": "9 Hari",
  //     "hotel": 4,
  //     "totalseat": 40,
  //     "remainingseat": 20,
  //     "lokasiberangkat": "Surakarta"
  //   },
  //   {
  //     "_id": {
  //       "$oid": "660f52bd9a7feaedf622ac3c"
  //     },
  //     "key": "paket_5",
  //     "img": "/Poster/paket_5.jpeg",
  //     "harga": "Rp24.500.000,~",
  //     "tipe": "Quad",
  //     "title": "Umroh Eksekutif Akhir Desember 2024 Plus City tour UIM + Museum Wahyu",
  //     "jadwal": "27 Dec 24",
  //     "durasi": "9 Hari",
  //     "hotel": 4,
  //     "totalseat": 40,
  //     "remainingseat": 5,
  //     "lokasiberangkat": "Surakarta"
  //   }]
  
  // KENAPA KAMI
  export const KENAPA_KAMI = [
    {
      key: 'hemat',
      title: 'HEMAT',
      icon: '/hemat.svg',
      description:
        'Memberikan harga terjangkau dengan pilihan paket yang variatif. Serta Promo dan Diskon menarik lainnya, yang ga bikin tabungan kamu jadi kurus.',
    },
    {
      key: 'nyaman',
      title: 'NYAMAN',
      icon: '/nyaman.svg',
      description:
        "Melayani Calon Jamaah dan Jama’ah dengen tulus. Memberikan tips-tips serta bimbingan dari Staf yang terlatih agar anda merasa nyaman dalam berbagai traveling, kuliner halal bersama dengan Albayt Wisata.",
    },
    {
      key: 'aman',
      title: 'AMAN',
      icon: '/aman.svg',
      description:
        'Albayt memastikan kepada Jama’ah dan Calon Jama’ah, bahwa kami adalah Travel yang Berizin dan Amanah. Dengan Jadwal yang Pasti, Sehingga Anda merasa aman memilih kami sebagai Travel Perjalanan anda.',
    },
  ];

  // PEMBIMBING 
  export const DUMMY_PEMBIMBING = [
    {
      key: 'pembimbing_1',
      image: "/images/pembimbing1.png",
      nama: "Ustad Fulan",
    },
    {
      key: 'pembimbing_2',
      image: "/images/pembimbing1.png",
      nama: "Ustad El-Fulan",
    },
    {
      key: 'pembimbing_3',
      image: "/images/pembimbing1.png",
      nama: "Ustad Fulano",
    },
    {
      key: 'pembimbing_4',
      image: "/images/pembimbing1.png",
      nama: "Ustad Fulano bin Fulana",
    },
    {
      key: 'pembimbing_5',
      image: "/images/pembimbing1.png",
      nama: "Ustad Al-Fulana II",
    },
    {
      key: 'pembimbing_6',
      image: "/images/pembimbing1.png",
      nama: "Ustad Fulano el Fulani",
    },
    {
      key: 'pembimbing_7',
      image: "/images/pembimbing1.png",
      nama: "Ustad Fulana Fulano",
    },
    {
      key: 'pembimbing_8',
      image: "/images/pembimbing1.png",
      nama: "Ustad Brother Fulana",
    },
  ]

  export const TESTIMONI = [
    {
      "key": "testi1",
      "img": "/images/sample-hajj1.png",
      "nama": "Ibu Eti",
      "bintang": 5,
      "review": "Perjalanan wisata ke Aqsa dengan Ibu saya bersama ALBAYT sangat berkesan. Dengan Staff yang berpengalaman, saya dan Ibu saya merasa tenang dan nyaman."
    },
    {
      "key": "testi2",
      "img": "/images/sample-hajj2.png",
      "nama": "Eyang Nirlam & Eyang Unay",
      "bintang": 5,
      "review": "Terima Kasih untuk panitia Albayt & Pak Ust Jazuli yang telah membimbing kami. Pak Ust Jazuli sangat mengayomi & membimbing eyang dengan sangat baik, penuh kesabaran."
    },
    {
      "key": "testi3",
      "img": "/images/sample-hajj3.png",
      "nama": "Eyang Omayanti",
      "bintang": 5,
      "review": "Terima Kasih kepada panitia perjalanan umroh milad, sudah memberikan pelayanan terbaik kepada eyang omayanti. sangat membantu kami dari mulai persiapan keberangkatan. Dan kepada Ustad H. Jazuli yang telah membimbing eyang dengan baik , sabar."
    },
    {
      "key": "testi4",
      "img": "/images/sample-hajj1.png",
      "nama": "Nenek Enong",
      "bintang": 5,
      "review": "Pembimbing dan peserta lainnya sangat baik."
    },
    {
      "key": "testi5",
      "img": "/images/sample-hajj2.png",
      "nama": "Awiez, Founder QUINSHA STUFF",
      "bintang": 5,
      "review": "WIsata Muslim bersama ALBAYT WISATA sangat nyaman, dengan pelayanan yang ramah dan harga yang bersahabat."
    },
    {
      "key": "testi5",
      "img": "/images/sample-hajj2.png",
      "nama": "Awiez, Founder QUINSHA STUFF",
      "bintang": 5,
      "review": "WIsata Muslim bersama ALBAYT WISATA sangat nyaman, dengan pelayanan yang ramah dan harga yang bersahabat."
    },
    {
      "key": "testi5",
      "img": "/images/sample-hajj2.png",
      "nama": "Awiez, Founder QUINSHA STUFF",
      "bintang": 5,
      "review": "WIsata Muslim bersama ALBAYT WISATA sangat nyaman, dengan pelayanan yang ramah dan harga yang bersahabat."
    },
    {
      "key": "testi5",
      "img": "/images/sample-hajj2.png",
      "nama": "Awiez, Founder QUINSHA STUFF",
      "bintang": 5,
      "review": "WIsata Muslim bersama ALBAYT WISATA sangat nyaman, dengan pelayanan yang ramah dan harga yang bersahabat."
    },
    {
      "key": "testi5",
      "img": "/images/sample-hajj2.png",
      "nama": "Awiez, Founder QUINSHA STUFF",
      "bintang": 5,
      "review": "WIsata Muslim bersama ALBAYT WISATA sangat nyaman, dengan pelayanan yang ramah dan harga yang bersahabat."
    },
    {
      "key": "testi5",
      "img": "/images/sample-hajj2.png",
      "nama": "Awiez, Founder QUINSHA STUFF",
      "bintang": 5,
      "review": "WIsata Muslim bersama ALBAYT WISATA sangat nyaman, dengan pelayanan yang ramah dan harga yang bersahabat."
    },
    {
      "key": "testi5",
      "img": "/images/sample-hajj2.png",
      "nama": "Awiez, Founder QUINSHA STUFF",
      "bintang": 5,
      "review": "WIsata Muslim bersama ALBAYT WISATA sangat nyaman, dengan pelayanan yang ramah dan harga yang bersahabat."
    },
    {
      "key": "testi5",
      "img": "/images/sample-hajj2.png",
      "nama": "Awiez, Founder QUINSHA STUFF",
      "bintang": 5,
      "review": "WIsata Muslim bersama ALBAYT WISATA sangat nyaman, dengan pelayanan yang ramah dan harga yang bersahabat."
    },
    {
      "key": "testi5",
      "img": "/images/sample-hajj2.png",
      "nama": "Awiez, Founder QUINSHA STUFF",
      "bintang": 5,
      "review": "WIsata Muslim bersama ALBAYT WISATA sangat nyaman, dengan pelayanan yang ramah dan harga yang bersahabat."
    },
    {
      "key": "testi5",
      "img": "/images/sample-hajj2.png",
      "nama": "Awiez, Founder QUINSHA STUFF",
      "bintang": 5,
      "review": "WIsata Muslim bersama ALBAYT WISATA sangat nyaman, dengan pelayanan yang ramah dan harga yang bersahabat."
    },
    {
      "key": "testi5",
      "img": "/images/sample-hajj2.png",
      "nama": "Awiez, Founder QUINSHA STUFF",
      "bintang": 5,
      "review": "WIsata Muslim bersama ALBAYT WISATA sangat nyaman, dengan pelayanan yang ramah dan harga yang bersahabat."
    }
  ]
  
  // FOOTER SECTION
  export const FOOTER_LINKS = [
    {
      key: 'learn_more',
      title: 'Learn More',
      links: [
        {label: 'about', value: 'About Albayt Solo', link: '/about-us'},
        {label: 'privacy', value: 'Privacy Policy', link: '/'},
        {label: 'contact', value: 'Contact Us', link: 'https://wa.me/628122586045'},
      ],
    },
  ];
  
  export const FOOTER_CONTACT_INFO = {
    key: 'footer_contact',
    title: 'Contact Us',
    links: [
      { label: 'Admin Officer', value: '+62-812-2586-045', link: 'https://wa.me/628122586045' },
      { label: 'Email Officer', value: 'albaytsolo@gmail.com', link: 'mailto:albaytsolo@gmail.com' },
    ],
  };
  
  export const SOCIALS = {
    key: 'social',
    title: 'Social',
    links: [
      {label: 'facebook', value:'/facebook.svg'},
      {label: 'instagram', value: '/instagram.svg'},
      {label: 'tiktok', value: '/tiktok.svg'},
      {label: 'youtube', value: '/youtube.svg'},
    ],
  };

  export const VISI_MISI = [
    {
      content_key: 'visi',
      title: 'VISI',
      content: [
        {
          label: 'visi1', value: 'Meraih keberkahan dalam berwisata.',
        },
        {
          label: 'visi2', value: 'Menjadi perusahaan jasa tour & travel muslim secara komprehensif & profesional dengan berorientasi kepada kepuasan pelanggan.',
        },
        {
          label: 'visi3', value: 'Menjadi biro perjalanan umrah & haji yang mengedepankan bimbingan ibadah yang khusuk dengan pelayanan maksimal, harga terjangkau, dan amanah.',
        },
      ],
    },
    {
      content_key: 'misi',
      title: 'MISI',
      content: [
        {
          label: 'misi1', value: 'Membantu memudahkan perencanaan perjalanan wisata yang ekonomis dan nyaman.',
        },
        {
          label: 'misi2', value: 'Memberikan harga yang sangat terjangkau dengan pilihan paket yang variatif.',
        },
        {
          label: 'misi3', value: 'Memberikan kenyamanan dan bimbingan beribadah umrah dan haji untuk mencapai mabrur.',
        },
        {
          label: 'misi4', value: 'Membina komunikasi yang baik dengan semua pelanggan.',
        },
        {
          label: 'misi5', value: 'Menjalin kerjasama dalam bidang pariwisata dengan semua pihak terkait.'
        },
        {
          label: 'misi6', value: 'Menggunakan SDM / tenaga kerja yang terlatih, bersertifikat, dan profesional.'
        },
        {
          label: 'misi7', value: 'Meningkatkan pelayanan yang terbaik demi kepuasan pelanggan.'
        },
      ],
    },
  ]

  // export const AGEN_PERWAKILAN = [
  //   {
  //     key: 'agen1',
  //     title: 'KALIMANTAN BARAT' ,
  //     lokasi: 'Jln. Lorem Ipsum Dolor Sit Amet No 7 Gang 18',
  //     pj: 'Bapak Lorem Ipsum',
  //     telp: '123-456-789',
  //   },
  //   {
  //     key: 'agen2',
  //     title: 'KARESIDENAN MADIUN' ,
  //     lokasi: 'Jln. Lorem Ipsum Dolor Sit Amet No 7 Gang 18',
  //     pj: 'Bapak Lorem Ipsum',
  //     telp: '123-456-789',
  //   },
  //   {
  //     key: 'agen3',
  //     title: 'BEKASI PUSAT' ,
  //     lokasi: 'Jln. Lorem Ipsum Dolor Sit Amet No 7 Gang 18',
  //     pj: 'Bapak Lorem Ipsum',
  //     telp: '123-456-789',
  //   },
  //   {
  //     key: 'agen4',
  //     title: 'SURAKARTA' ,
  //     lokasi: 'Jln. Lorem Ipsum Dolor Sit Amet No 7 Gang 18',
  //     pj: 'Bapak Lorem Ipsum',
  //     telp: '123-456-789',
  //   },
  // ]

  export const FOLDER_GALERI = [
    {
      folder_key: 'folder1',
      folder_link: '/galeri-folder1',
      thumbnail: '/images/sample-hajj1.png',
      title: 'UMROH BERSAMA 2024',
      desc: 'Lorem Ipsum Dolor Sit Amet',
    },
    {
      folder_key: 'folder2',
      folder_link: '/galeri-folder1',
      thumbnail: '/images/sample-hajj2.png',
      title: 'UMROH BERSAMA 2023',
      desc: 'Lorem Ipsum Dolor Sit Amet',
    },
    {
      folder_key: 'folder3',
      folder_link: '/galeri-folder1',
      thumbnail: '/images/sample-hajj3.png',
      title: 'UMROH BERSAMA 2022',
      desc: 'Lorem Ipsum Dolor Sit Amet',
    },
  ]

  export const GALERI_2024 = [
    {
      key: 'image1',
      link: '/images/sample-hajj1.png',
      title: 'Lorem Ipsum Dolor',
    },
    {
      key: 'image2',
      link: '/images/sample-hajj2.png',
      title: 'Lorem Ipsum Dolor',
    },
    {
      key: 'image3',
      link: '/images/sample-hajj3.png',
      title: 'Lorem Ipsum Dolor',
    },
    {
      key: 'image4',
      link: '/images/sample-hajj1.png',
      title: 'Lorem Ipsum Dolor',
    },
    {
      key: 'image5',
      link: '/images/sample-hajj2.png',
      title: 'Lorem Ipsum Dolor',
    },
    {
      key: 'image6',
      link: '/images/sample-hajj3.png',
      title: 'Lorem Ipsum Dolor',
    },
    {
      key: 'image7',
      link: '/images/sample-hajj1.png',
      title: 'Lorem Ipsum Dolor',
    },
    {
      key: 'image8',
      link: '/images/sample-hajj2.png',
      title: 'Lorem Ipsum Dolor',
    },
    {
      key: 'image9',
      link: '/images/sample-hajj3.png',
      title: 'Lorem Ipsum Dolor',
    },
    {
      key: 'image10',
      link: '/images/sample-hajj1.png',
      title: 'Lorem Ipsum Dolor',
    },
    {
      key: 'image11',
      link: '/images/sample-hajj2.png',
      title: 'Lorem Ipsum Dolor',
    },
    {
      key: 'image12',
      link: '/images/sample-hajj3.png',
      title: 'Lorem Ipsum Dolor',
    },
  ]

  export const BERITA_ARTIKEL = [
    {
      id: "000001",
      title: "Contoh Artikel",
      category: "Livestyle",
      author: "Penulis Blog 1",
      date: "2023-11-14",
      image: "https://picsum.photos/201",
      content: `
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 1</h2>
        <p class="text-base mb-4">Lorem ipsum dolor sit amet, consectetur adipiscing elit. In porta ultricies mollis. Cras dignissim pulvinar pharetra. Etiam viverra turpis metus, ullamcorper rutrum nisl molestie id. Nulla tincidunt nisi magna, ut venenatis ipsum vehicula id. Integer sit amet tincidunt erat, nec aliquam nunc. Aenean sollicitudin ex sit amet tellus varius efficitur. Quisque posuere velit nunc, ac pulvinar urna venenatis a. Aliquam tempor, quam non hendrerit consectetur, augue felis tempor nulla, non vestibulum magna tellus vitae sem. Cras vestibulum tellus dolor, non dignissim elit maximus eget. Nunc in enim imperdiet, finibus est quis, vulputate augue. Vivamus in dui vestibulum, euismod nisi in, dignissim mauris. Ut at justo pharetra, tincidunt ipsum a, molestie orci. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam fermentum ante a ipsum mattis, vel porta ante ultricies. Aenean vitae lorem lorem.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 2</h2>
        <p class="text-base mb-4">Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 3</h2>
        <p class="text-base mb-4">Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
        <ul class="list-disc list-inside mb-4">
          <li>List Item 1</li>
          <li>List Item 2</li>
          <li>List Item 3</li>
        </ul>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 4</h2>
        <p class="text-base mb-4">Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p>
      `,
    },
    {
      id: "000002",
      title: "Contoh Artikel",
      category: "Livestyle",
      author: "Penulis Blog 1",
      date: "2023-11-14",
      image: "https://picsum.photos/202",
      content: `
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 1</h2>
        <p class="text-base mb-4">Lorem ipsum dolor sit amet, consectetur adipiscing elit. In porta ultricies mollis. Cras dignissim pulvinar pharetra. Etiam viverra turpis metus, ullamcorper rutrum nisl molestie id. Nulla tincidunt nisi magna, ut venenatis ipsum vehicula id. Integer sit amet tincidunt erat, nec aliquam nunc. Aenean sollicitudin ex sit amet tellus varius efficitur. Quisque posuere velit nunc, ac pulvinar urna venenatis a. Aliquam tempor, quam non hendrerit consectetur, augue felis tempor nulla, non vestibulum magna tellus vitae sem. Cras vestibulum tellus dolor, non dignissim elit maximus eget. Nunc in enim imperdiet, finibus est quis, vulputate augue. Vivamus in dui vestibulum, euismod nisi in, dignissim mauris. Ut at justo pharetra, tincidunt ipsum a, molestie orci. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam fermentum ante a ipsum mattis, vel porta ante ultricies. Aenean vitae lorem lorem.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 2</h2>
        <p class="text-base mb-4">Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 3</h2>
        <p class="text-base mb-4">Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
        <ul class="list-disc list-inside mb-4">
          <li>List Item 1</li>
          <li>List Item 2</li>
          <li>List Item 3</li>
        </ul>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 4</h2>
        <p class="text-base mb-4">Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p>
      `,
    },
    {
      id: "000003",
      title: "Contoh Artikel",
      category: "Livestyle",
      author: "Penulis Blog 1",
      date: "2023-11-14",
      image: "https://picsum.photos/203",
      content: `
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 1</h2>
        <p class="text-base mb-4">Lorem ipsum dolor sit amet, consectetur adipiscing elit. In porta ultricies mollis. Cras dignissim pulvinar pharetra. Etiam viverra turpis metus, ullamcorper rutrum nisl molestie id. Nulla tincidunt nisi magna, ut venenatis ipsum vehicula id. Integer sit amet tincidunt erat, nec aliquam nunc. Aenean sollicitudin ex sit amet tellus varius efficitur. Quisque posuere velit nunc, ac pulvinar urna venenatis a. Aliquam tempor, quam non hendrerit consectetur, augue felis tempor nulla, non vestibulum magna tellus vitae sem. Cras vestibulum tellus dolor, non dignissim elit maximus eget. Nunc in enim imperdiet, finibus est quis, vulputate augue. Vivamus in dui vestibulum, euismod nisi in, dignissim mauris. Ut at justo pharetra, tincidunt ipsum a, molestie orci. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam fermentum ante a ipsum mattis, vel porta ante ultricies. Aenean vitae lorem lorem.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 2</h2>
        <p class="text-base mb-4">Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 3</h2>
        <p class="text-base mb-4">Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
        <ul class="list-disc list-inside mb-4">
          <li>List Item 1</li>
          <li>List Item 2</li>
          <li>List Item 3</li>
        </ul>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 4</h2>
        <p class="text-base mb-4">Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p>
      `,
    },
    {
      id: "000004",
      title: "Contoh Artikel",
      category: "Livestyle",
      author: "Penulis Blog 1",
      date: "2023-11-14",
      image: "https://picsum.photos/204",
      content: `
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 1</h2>
        <p class="text-base mb-4">Lorem ipsum dolor sit amet, consectetur adipiscing elit. In porta ultricies mollis. Cras dignissim pulvinar pharetra. Etiam viverra turpis metus, ullamcorper rutrum nisl molestie id. Nulla tincidunt nisi magna, ut venenatis ipsum vehicula id. Integer sit amet tincidunt erat, nec aliquam nunc. Aenean sollicitudin ex sit amet tellus varius efficitur. Quisque posuere velit nunc, ac pulvinar urna venenatis a. Aliquam tempor, quam non hendrerit consectetur, augue felis tempor nulla, non vestibulum magna tellus vitae sem. Cras vestibulum tellus dolor, non dignissim elit maximus eget. Nunc in enim imperdiet, finibus est quis, vulputate augue. Vivamus in dui vestibulum, euismod nisi in, dignissim mauris. Ut at justo pharetra, tincidunt ipsum a, molestie orci. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam fermentum ante a ipsum mattis, vel porta ante ultricies. Aenean vitae lorem lorem.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 2</h2>
        <p class="text-base mb-4">Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 3</h2>
        <p class="text-base mb-4">Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
        <ul class="list-disc list-inside mb-4">
          <li>List Item 1</li>
          <li>List Item 2</li>
          <li>List Item 3</li>
        </ul>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 4</h2>
        <p class="text-base mb-4">Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p>
      `,
    },
    {
      id: "000005",
      title: "Contoh Artikel",
      category: "Livestyle",
      author: "Penulis Blog 1",
      date: "2023-11-14",
      image: "https://picsum.photos/205",
      content: `
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 1</h2>
        <p class="text-base mb-4">Lorem ipsum dolor sit amet, consectetur adipiscing elit. In porta ultricies mollis. Cras dignissim pulvinar pharetra. Etiam viverra turpis metus, ullamcorper rutrum nisl molestie id. Nulla tincidunt nisi magna, ut venenatis ipsum vehicula id. Integer sit amet tincidunt erat, nec aliquam nunc. Aenean sollicitudin ex sit amet tellus varius efficitur. Quisque posuere velit nunc, ac pulvinar urna venenatis a. Aliquam tempor, quam non hendrerit consectetur, augue felis tempor nulla, non vestibulum magna tellus vitae sem. Cras vestibulum tellus dolor, non dignissim elit maximus eget. Nunc in enim imperdiet, finibus est quis, vulputate augue. Vivamus in dui vestibulum, euismod nisi in, dignissim mauris. Ut at justo pharetra, tincidunt ipsum a, molestie orci. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam fermentum ante a ipsum mattis, vel porta ante ultricies. Aenean vitae lorem lorem.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 2</h2>
        <p class="text-base mb-4">Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 3</h2>
        <p class="text-base mb-4">Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
        <ul class="list-disc list-inside mb-4">
          <li>List Item 1</li>
          <li>List Item 2</li>
          <li>List Item 3</li>
        </ul>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 4</h2>
        <p class="text-base mb-4">Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p>
      `,
    },
    {
      id: "000006",
      title: "Contoh Artikel",
      category: "Livestyle",
      author: "Penulis Blog 1",
      date: "2023-11-14",
      image: "https://picsum.photos/206",
      content: `
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 1</h2>
        <p class="text-base mb-4">Lorem ipsum dolor sit amet, consectetur adipiscing elit. In porta ultricies mollis. Cras dignissim pulvinar pharetra. Etiam viverra turpis metus, ullamcorper rutrum nisl molestie id. Nulla tincidunt nisi magna, ut venenatis ipsum vehicula id. Integer sit amet tincidunt erat, nec aliquam nunc. Aenean sollicitudin ex sit amet tellus varius efficitur. Quisque posuere velit nunc, ac pulvinar urna venenatis a. Aliquam tempor, quam non hendrerit consectetur, augue felis tempor nulla, non vestibulum magna tellus vitae sem. Cras vestibulum tellus dolor, non dignissim elit maximus eget. Nunc in enim imperdiet, finibus est quis, vulputate augue. Vivamus in dui vestibulum, euismod nisi in, dignissim mauris. Ut at justo pharetra, tincidunt ipsum a, molestie orci. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam fermentum ante a ipsum mattis, vel porta ante ultricies. Aenean vitae lorem lorem.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 2</h2>
        <p class="text-base mb-4">Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 3</h2>
        <p class="text-base mb-4">Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
        <ul class="list-disc list-inside mb-4">
          <li>List Item 1</li>
          <li>List Item 2</li>
          <li>List Item 3</li>
        </ul>
        <h2 class="text-lg font-bold mt-4 mb-2">Subjudul 4</h2>
        <p class="text-base mb-4">Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p>
      `,
    },
  ];

  // export const RIWAYAT_PEMBELIAN = [
  //   {
  //     id: 1,
  //     tanggal: '9 Feb 2024',
  //     status: 'Berhasil',
  //     keterangan: 'Umroh Eksekutif Akhir Desember 2024 Plus City tour UIM + Museum Wahyu',
  //     total: 22001500,
  //   },
  //   {
  //     id: 2,
  //     tanggal: '9 Feb 2024',
  //     status: 'Berhasil',
  //     keterangan: 'Umroh Eksekutif Akhir Desember 2024 Plus City tour UIM + Museum Wahyu',
  //     total: 22001500,
  //   },
  //   {
  //     id: 3,
  //     tanggal: '9 Feb 2024',
  //     status: 'Berhasil',
  //     keterangan: 'Umroh Eksekutif Akhir Desember 2024 Plus City tour UIM + Museum Wahyu',
  //     total: 20125500,
  //   },
  // ];

  export const FASILITAS_PAKET = [
    {
      id_fasilitas: "include",
      title: "Include",
      contents: [
        {
          label: "tiket",
          value: "Tiket Pesawat PP Kelas Ekonomi",
        },
        {
          label: "visa",
          value: "Visa Umrah",
        },
        {
          label: "hotel_madinah",
          value: "Hotel Madinah *3/4 Setaraf (QUAD)",
        },
        {
          label: "hotel_mekah",
          value: "Hotel Makkah *3/4 Setaraf (Quad)",
        },
        {
          label: "makan",
          value: "Makan Fullboard",
        },
        {
          label: "transport",
          value: "Transportasi dan Akomodasi",
        },
        {
          label: "muthawif",
          value: "Muthawif dan TL",
        },
        {
          label: "handlinh",
          value: "Handlinh dan Perlengkapan",
        },
        {
          label: "manasik",
          value: "Manasik",
        },
      ], 
    },
    {
      id_fasilitas: "exclude",
      title: "Exclude",
      contents: [
        {label: "biaya", value: "Biaya Pembuatan Passport"},
        {label: "suntik", value: "Suntik Meningitis"},
        {label: "biaya_kirim", value: "Biaya Pengiriman Perlengkapan ke Daerah"},
        {label: "upgrade_1", value: "Biaya Upgrade Double Program 9-11 Hari: Double Room 3,5jt/PA, Triple 2jt/PAX"},
        {label: "upgrade_2", value: "Biaya Upgrade Double Program 12-16 Hari: Double Room 6jt/PA, Triple 3,5jt/PAX"},
        {label: "tipping", value: "Tipping Guide Turki/Cairo/Dubai (8USD Per Orang Per Hari)"},
        {label: "biaya_lain", value: "Biaya Lain Apabila Ada Ketentuan Kebijakan Pemerintah Indonesia / Saudi Arabia"},
        {label: "biaya_tiket", value: "Biaya Tiket Domestik"},
      ],
    },
    {
      id_fasilitas: "note",
      title: "Note",
      contents: [
        {label: "depo", value: "Deposit 5.000.000 / PAX (Non Refundable)"},
        {label: "pelunasan", value: "Pelunasan H-40"},
        {label: "harga_program", value: "Harga dan Program Sewaktu-waktu Bisa Berubah Tanpa Pemberitahuan"},
        {label: "penyesuaian", value: "Disesuaikan Dengan Situasi dan Kondisi Tanpa Mengurangi Kekhusukan Ibadah"},
        {label: "teman_kamar", value: "Jika Tidak Ada Teman Sekamar, Maka Wajib Membayar Upgrade Kamar"},
      ],
    },
  ]