...
const BoxPemesanan: React.FC<{ currentPaket: Paket, exchangeRate: number  }> = ({ currentPaket, exchangeRate }) => {
  ...
  return (
      <div>
        <div className="bg-white border-[rgba(0,0,0,0.16)] shadow-2xl flex flex-col justify-center items-center rounded-2xl max-w-sm w-[100%] h-fit p-10">
          <h1 className='font-bold text-center text-md mb-5'>{currentPaket?.title}</h1>
          ...
          <div className='border-gray-20 rounded-2xl px-4 my-3 w-full bg-slate-100 mb-6'>
            <div className='flex items-center my-2'>
              <div className='w-full text-2xl font-bold text-yellow-600 text-center'>
                <p>DP </p>
                <p>Rp{currentPaket?.harga_dp?.toLocaleString('id-ID', {style: 'currency', currency: 'IDR'})}</p>
              </div>
            </div>
            <div>
